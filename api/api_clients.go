...
func (r ApiGetClientInfoRequest) Execute() (*GetEntityClientPreCreateInfoResponse, *http.Response, error) {
...
*/
func (a *ClientsAPIService) GetClientInfo(ctx context.Context, companyId int32) ApiGetClientInfoRequest {
	return ApiGetClientInfoRequest{
		ApiService: a,
		ctx: ctx,
		companyId: companyId,
...
func (a *ClientsAPIService) GetClientInfoExecute(r ApiGetClientInfoRequest) (*GetEntityClientPreCreateInfoResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *GetEntityClientPreCreateInfoResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ClientsAPIService.GetClientInfo")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/c/{company_id}/entities/clients/info"
	localVarPath = strings.Replace(localVarPath, "{"+"company_id"+"}", url.PathEscape(parameterValueToString(r.companyId, "companyId")), -1)
