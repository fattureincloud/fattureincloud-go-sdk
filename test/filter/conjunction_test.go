...
func TestConjunction(t *testing.T) {
	...
	c := filter.NewConjunction(left, right)
	assert.Equal(t, left, c.Left)
	assert.Equal(t, right, c.Right)
	assert.Equal(t, "(city = 'Bergamo' and age < 35)", c.BuildQuery())
	assert.Equal(t, "(city = 'Bergamo' and age < 35)", c.String())

	left1 := filter.NewCondition("team", filter.Operators.EQ, "Volley Bergamo 1991")
	c.Left = left1
	assert.Equal(t, left1, c.Left)
	assert.Equal(t, "(team = 'Volley Bergamo 1991' and age < 35)", c.BuildQuery())
	assert.Equal(t, "(team = 'Volley Bergamo 1991' and age < 35)", c.String())

	right1 := filter.NewCondition("best_player", filter.Operators.IS_NOT, nil)
	c.Right = right1
	assert.Equal(t, right1, c.Right)
	assert.Equal(t, "(team = 'Volley Bergamo 1991' and best_player is not null)", c.BuildQuery())
	assert.Equal(t, "(team = 'Volley Bergamo 1991' and best_player is not null)", c.String())
}
